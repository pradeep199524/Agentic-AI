...
class PostgresSQLAgent:
    ...
    async def execute_query(self, query: str, llm_client: Union[AsyncOpenAI, AsyncAzureOpenAI], model: str) -> List[str]:
        ...
        system_prompt = (
            ...
            "STRICT RULES:\n"
            "1. FULL DATA ACCESS: ALWAYS use `SELECT *` so all columns and numeric metrics are returned.\n"
            "2. NO ROW RESTRICTIONS: DO NOT use `LIMIT 1` or cut off rows. Return the full table or all matching rows so the evaluation LLM has complete context to decide.\n"
            "3. OPTIONAL ORDERING: You may include `ORDER BY column_name DESC` or `ASC` to organize the data logically, but do NOT add `LIMIT`.\n"
